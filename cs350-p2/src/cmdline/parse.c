...
int
parse_command_line (int argc, char **argv,const char** filename, bool
*sflag,bool *iflag,const char** soptarg)
{
   // printf ("Replace this with code that calls getopt();\n");
	...
	while((c = getopt(argc,argv,"s:i"))!= -1)
	{
		...
		case 's':
		printf("implement flag capture for symbol list\n");
		*sflag = true;
		*soptarg = optarg;
		break;
		case 'i':
		printf("Implement flag capture for information file\n");
		*iflag = true;
		break;
		case '?':
		if(optopt == 'c')
		...
		if(argc == 1)
		{
			printf("NO ARGUMENTS BEING PASSED.. EXITING NOW..\n");
			exit(1);
